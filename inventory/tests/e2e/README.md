# E2E тесты для Inventory сервиса

Этот пакет содержит end-to-end тесты для Inventory сервиса, использующие testcontainers для создания изолированного тестового окружения.
...
- `inventory_test.go` - тесты всех gRPC ручек Inventory сервиса
...
```go
It("should create Part successfully", func() {
    // создаем Part
    // проверяем результат
...
var _ = Describe("Inventory Service", func() {
    // Здесь все тесты про Inventory сервис
})
```

#### 2. Context - "В какой ситуации"

Это описание конкретной ситуации или сценария. Context помогает группировать тесты по похожим условиям. Например, "когда создаем новую деталь" или "когда деталь уже существует".

```go
Context("when creating new part", func() {
    // Тесты про создание новой детали
})

Context("when part already exists", func() {
    // Тесты про работу с существующими деталями
})
```
...
#### Полный пример нашего проекта:

Читается как: "Inventory Service, когда выполняется операция создания, должен создать деталь успешно"

```go
var _ = Describe("Inventory Service E2E Tests", func() {
    ...
    Context("Get operation", func() {
        It("should retrieve existing part", func() {
            // Тест получения
        })
    })
})
...
var _ = BeforeSuite(func() {
    ...
    inventoryContainer = startInventoryContainer()
})
```
...
**Проверка равенства:**
```go
Expect(response.Info.Name.GetValue()).To(Equal("Test Name"))
// "Ожидаю, что имя будет равно 'Test Name'"
```
...
```go
Expect(response.Info.Name.GetValue()).To(ContainSubstring("Engine"))
// "Ожидаю, что название содержит слово 'Engine'"
```
...
func TestE2E(t *testing.T) {
    ...
    RunSpecs(t, "Inventory E2E Tests")  // Запускаем все тесты
}
```
...
var _ = BeforeSuite(func() {
    ...
    // Запускаем Inventory сервис контейнер
    // Ждем пока все поднимется
})
```

#### 3. Файл `inventory_test.go` - сами тесты
```go
var _ = Describe("Inventory Service E2E Tests", func() {
    BeforeEach(func() {
        ClearPartsCollection() // Очищаем базу перед каждым тестом
    })
...
Inventory Service E2E Tests
  Create operation
    ✓ should create part successfully [0.123 seconds]
  Get operation  
    ✓ should retrieve existing part [0.089 seconds]
  Update operation
    ✓ should update part successfully [0.156 seconds]
...
#### Create (создание детали)
- ✅ Успешное создание детали с валидными данными
- ✅ Генерация корректного UUID
- ✅ Сохранение в MongoDB

#### Get (получение детали)
- ✅ Успешное получение детали по UUID
- ✅ Корректное возвращение всех полей
- ✅ Правильная обработка timestamps

#### Update (обновление детали)
- ✅ Успешное обновление существующей детали
- ✅ Корректное обновление всех полей (name, price, stockQuantity, category, dimensions, manufacturer, tags, metadata)
- ✅ Установка updated_at timestamp

#### Delete (удаление детали)
...
1. **Docker сеть** - создается отдельная сеть `inventory-service`
2. **MongoDB контейнер** - база данных с аутентификацией
3. **Inventory сервис контейнер** - собирается из `deploy/docker/inventory/Dockerfile`
...
4. Используйте `task e2e:test:inventory` для запуска с правильными переменными
